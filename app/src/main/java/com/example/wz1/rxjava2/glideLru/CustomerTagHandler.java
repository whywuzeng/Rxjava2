...
import com.example.wz1.rxjava2.App;
...
public class CustomerTagHandler implements Html.TagHandler {
    private static final String TAG = "CustomerTagHandler";
    /**
     * html 标签的开始下标
     */
    ...
    private void handlerEndSIZE(Editable output) {
        ...
        if (!isEmpty(propertyValue)) {
            try {
                ...
                output.setSpan(new AbsoluteSizeSpan(sp2px(App.getInstance(), value)), startIndex.pop(), output.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
